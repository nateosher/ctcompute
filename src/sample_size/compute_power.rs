...
pub fn compute_power(
    ...
    let z_alpha = std_normal_quantile(1. - alpha)?;

    //    theta = z_alpha + z_beta
    //    theta = delta * \sqrt(I(1))
    // => z_alpha = delta*\sqrt(I(1)) - z_beta
    // Except some signs are flipped because theta is assumed to be
    // negative
    let z_one_minus_beta = log_hr * implied_I.sqrt() + z_alpha;

    Ok(std_normal_cdf(-z_one_minus_beta))
}
...
mod tests {
    ...
    fn compute_power_1() {
        ...
        assert!((power - 0.9) < 0.002);
        // Since we "round up" events needed, information needed,
        // etc., alpha should be slightly larger than nominal trial
        // power
        assert!(power > 0.9);
    }
}
